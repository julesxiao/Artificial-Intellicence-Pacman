...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    #check if a ghost has been captured by Pacman
    tempProb = allPossible
    if noisyDistance == None :
      #all beliefs should be updated so that the ghost appears in its prison cell, position self.getJailPosition()
        tempProb[self.getJailPosition()] = 1
    else:
        for p in self.legalPositions:
          trueDistance = util.manhattanDistance(p, pacmanPosition)
          if emissionModel[trueDistance] > 0:
              tempE = emissionModel[trueDistance]
              tempBelief = self.beliefs[p]
              # p(X = x|e) <-  p(X = x)p(e|X = x)
              tempProb[p] = tempBelief * tempE
    #print "allPossible ", allPossible
    tempProb.normalize()
    #print "self.beliefs ", self.beliefs
    self.beliefs = tempProb
  ...
  def elapseTime(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    tempProb = util.Counter()
    # for each previous ghost position
    for PositionOld in self.legalPositions :
      newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, PositionOld))
      #print "O_o!newPosDist ", newPosDist
      for PositionNew in newPosDist:
        #print "O_o!p : tempProb[p]", p,tempProb[p]
        #print "self.beliefs[PositionOld]", self.beliefs[PositionOld]
        #print "newPosDist[PositionNew]", newPosDist[PositionNew]
        probOfNewPosition = newPosDist[PositionNew]
        tempProb[PositionNew] = tempProb[PositionNew] + probOfNewPosition * self.beliefs[PositionOld]
    self.beliefs = tempProb
...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    for p in range(ParNum):
      itera = int(random.uniform(0,length))
      tempPro.update({self.legalPositions[itera]:1.0})
    #print "tempPro", tempPro
    tempPro.normalize()
    self.particles = tempPro
    self.beliefs = tempPro
    ...
    if noisyDistance == None:
      ...
      for p in self.legalPositions:
        ...
        if emissionModel[trueDistance] > 0:
          ...
          flagWeight = flagWeight + 1
    #call initializeUniformly.
    if flagWeight - ParNum is 0:
      self.initializeUniformly(gameState)
      tempProb = self.beliefs

    tempProb.normalize()
    #print "tempProb ",tempProb
    self.beliefs = tempProb
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    for p in posLegal: tempPro[p] = 1.0
    
    newParticles = []
    for particle in range(ParNum):
      newParticle = []
      for eachGhost in range(self.numGhosts):
        newParticle.append(util.sample(tempPro))
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
      ...
      "*** YOUR CODE HERE ***"
      for eachGhost in range(ghostNum):
      #Each particle is a tuple of ghost positions
        newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle),eachGhost, self.ghostAgents[eachGhost])
        #print "newPosDist", newPosDist
        newParticle[eachGhost] = util.sample(newPosDist)
        #print "util.sample(newPosDist)", util.sample(newPosDist)
      #print "tuple(newParticle)", tuple(newParticle)
      newParticles.append(tuple(newParticle))
    self.particles = newParticles
    ...
    for eachSample in self.particles:
      ...
      for eachGhost in range(0, self.numGhosts):
        if tempPro2[self.getJailPosition(eachGhost)] == 1:
          ...
          trueDistance = util.manhattanDistance(eachSample[eachGhost], pacmanPosition) 
          weightParticle = weightParticle*emissionModels[eachGhost][trueDistance]
