...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    temp = 100000
    for ghostPosDis in livingGhostPositionDistributions:
      #the maze distance between any two positions
      mazeDis = self.distancer.getDistance(pacmanPosition, ghostPosDis.argMax())
      if mazeDis < temp:
        #find the nearest ghost
        nearest = ghostPosDis.argMax()
        temp = mazeDis
    #choose an action
    temp2 = 100000
    for pacmanAction in legal:
      mazeDis = self.distancer.getDistance( Actions.getSuccessor(pacmanPosition, pacmanAction), nearest)
      if mazeDis < temp2:
        nextAction = pacmanAction
        temp2 = mazeDis
